...
const OrgRateLimitPanel = ({ rows = [] }) => {
  return (
    <section className="rounded-xl border border-[#1e2a38] bg-[#0d1117] p-3">
      <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-[#cbd5e1]">Org Rate Limits</h3>
      <div className="mt-3 space-y-2">
        {rows.slice(0, 6).map((row) => {
          ...
          return (
            <div key={row.orgId} className="rounded-lg border border-[#1e2a38] bg-[#131920] px-2.5 py-2">
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs text-[#e2e8f0]">{row.name}</p>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-[#94a3b8]">{row.reqPerMin} req/min</span>
