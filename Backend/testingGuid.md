...
Expected: `201 Created` with a user object, auth cookies, and **`verificationToken`** (raw token for email verification).

For testing, you can immediately use the returned `verificationToken` with the verify-email endpoint instead of accessing emails.
...
Refresh token testing is based on the `refreshToken` cookie that is issued during login or register. The server validates it by calling `verifyRefreshToken(token)` in [Backend/src/utils/token.js](Backend/src/utils/token.js), then checks the stored hashed token record before rotating it.

What the server checks:

1. The `refreshToken` cookie exists.
2. The JWT signature is valid for the refresh secret.
3. The decoded payload has `type=refresh`.
4. The hashed token exists in the `RefreshToken` collection.
5. The token is not revoked and has not expired.
6. A new access token and refresh token are issued, and the previous refresh token is revoked.

Run the refresh request with the saved HTTPie session:

```bash
http --session=auth POST $BASE_URL/api/v1/auth/refresh
```

Expected: `200 OK` with `Token refreshed successfully` and rotated cookies.

Common failure cases:

- `401 Refresh token is required` when no cookie is present.
- `401 Invalid or expired refresh token` when the JWT cannot be verified.
- `401 Invalid refresh token` when the token payload is not a refresh token.
- `401 Refresh token is no longer valid` when the DB record is missing, revoked, or expired.
...
Expected: `200 OK` with a generic success message and **`resetToken`** (raw token for password reset).

For testing, you can immediately use the returned `resetToken` with the reset-password endpoint instead of accessing emails.
